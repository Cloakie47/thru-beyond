...
| 2026-08-02 | Max call depth is **15** (root + 14 nested); attempting 16 fails, invoke returns −24 | SDK header comment says "16 call depths (1..16)" — off by one vs observed | Budget for 15 |
| 2026-08-02 | Marginal hop residual ~487 CU ≈ callee path (~230) + ~256 — consistent with the invoke syscall's documented 256-byte register save charged as writes | Invoke effectively costs ≈ 512 + 256, more than the bare base | UNVERIFIED split; needs an isolating experiment |
...
| 2026-08-02 | **Compressed accounts are one-way black holes on alphanet**: `account compress` succeeds and removes the account from live state, then `prepare-decompression`, `account decompress`, and `make-state-proof existing/updating` ALL fail with RPC "bintrie: key not found", immediately and after 60+ s | Query service cannot find compressed leaves the chain just accepted | Do not compress anything you need back; three disposable test accounts sacrificed to establish this |
| 2026-08-02 | Compression: ≈ 6,053 + **1 CU per byte** of account data (6,061/7,117/11,053/71,621 at 8/1,000/5,000/65,536 B), SU = 0 at every size | Account is hashed at 1 CU/byte; **no state-unit refund — compression is validator-side, not economic for the payer** | The scaling-story economics question, answered |
