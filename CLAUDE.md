...
| 2026-08-02 | **Ephemeral accounts survive the proof outage**: created 3× (6,303 CU, SU 0, `Is Ephemeral: Yes`) while permanent creation failed −23 in the same session; transfer into one reverts (cannot hold funds) | `tsys_account_create_ephemeral(idx, seed)` touches no proof path | The only working creation primitive during a proof-service incident; GC/compress-delete still blocked by the −43 proof-shape mystery |
| 2026-08-02 | **`txn get` has always printed `Memory Units Consumed`** — and reports full consumed figures for REVERTED transactions too (the −23 deploy: CU 35,076, MU 1) | Our own grep filters (`compute\|state.units\|pages`) never matched the line; two filed issues (#36, #39) carried over-broad "not reported" premises from those filtered views | **Before claiming something is unavailable, enumerate every surface — each CLI subcommand, `--json`, Explorer JSON-RPC — and scope the claim to the surfaces actually checked with unfiltered output.** Corrections posted on both issues |
| 2026-08-02 | Example 10 measured via the **borrow-upgrade pattern** (04B's slot upgraded to the 10 binary during the desync): 18 CU/block-read, window exactly 512 (511 ok, 512 faults), record matches the Explorer byte-for-byte incl. the full nanosecond timestamp | Upgrades don't need creating proofs; program identity ≠ example identity when borrowing | 04B restorable by re-upgrade; noted in both READMEs |
| 2026-08-02 | Backfilled `results.json` for every example + `bench/verify.py` recomputing all published laws: **0 discrepancies** across ~90 checks | The verifier would have caught both earlier formula errors automatically | Run it after every session that touches figures |
| 2026-08-02 | **Declaring accounts costs 0 CU** — touch_none flat at 4,947 from N=0 to N=1,018, and nonexistent derived addresses are accepted read-only | Account-list validation/sorting is unmetered (submitter pre-sorts; chain verifies) | The predicted ~32 CU/entry never appeared; per-account costs start only on access: 44 CU/read, 590 CU/write |
| 2026-08-02 | **The size limit binds before the account limit**: envelope is 184 B, so max declared accounts = 1,018 (32,768−184)/32; failures split three ways (RPC size-reject ≥1,019; CLI signing refusal at 1,023–1,024; CLI validation at 1,025) | 1,024 × 32 B = the entire txn budget — the two headline limits collide by construction | The advertised 1,024 is unreachable by 6 even for an empty program call |
| 2026-08-02 | `--readwrite-accounts`/`--readonly-accounts` do NOT take comma lists — repeat the flag per address (a 44,649-char "address" error taught this) | clap multi-flag, not delimiter-split | 1,000+ repeated flags work fine |
| 2026-08-02 | **-O verdict**: -O1 runtime is within 0.12% of -O3 (224,775 vs 224,511) at 35% of the size; -Os/-Oz dominated by -O1 on both axes; -O0 = 4.27×; upgrade pipeline ≈137,149 + 171.1 CU/byte (see 2026-08-03 row) | Skill now recommends -O1 over the SDK's -O3 default | 04A cycled through all six levels and restored; restoration reproduced 224,511 exactly |
| 2026-08-02 | Ephemeral fleet: **950 accounts created in one transaction** (2,480,667 CU ≈ 2,611 each), and ephemeral resizes are ~SU-free (SU 2 for 950 resizes vs 1/page for permanent) | Ephemeral state isn't billed as persistent state | Fleet-scale scratch state is cheap and proof-independent |
| 2026-08-02 | WSL `/tmp` does not survive instance restarts (Ubuntu tmpfiles cleanup) — the O-level binaries vanished between sessions | Build artifacts belong on persistent storage (`build/` in the repo mount) | Cost one wasted measurement cycle |
...
| 2026-08-03 | The published "upgrade ≈170–195 CU/byte" didn't reproduce its own points (345,340/1,216 = 284) — **the THIRD time a constant was absorbed into a slope** (compress law, decompress law, now upgrade law) | Quoting a per-byte rate for a line with a 137k intercept; the first two corrections were never generalized into a repo-wide habit | Refit over all six points: **≈137,149 + 171.1 CU/byte, residuals ≤ ±0.43%** (approximate — same-size binaries differ 1,280 CU). Rule: never publish a per-X rate without stating the intercept; verify.py now checks the fit |
| 2026-08-03 | `tsys_increment_anonymous_segment_sz` (0x01) works: 512 + 4,096/page grown (+1 pg 38,791, +4 pg 51,079 over 34,127 re-baseline, 3× each); +100 B delta rejects −28 | Page-granular like its 0x00 sibling | Second data point that the anonymous-segment API family is page-granular everywhere |
| 2026-08-03 | Program-side `tsys_account_transfer` is **owner-authorized only**: owned-account → fee payer works (5,453 CU, 3×, balance verified); fee payer → anywhere reverts −10 | Programs may spend only accounts they own; the fee payer's balance is not program-spendable | The 09 payout projection's per-transfer term (512 base) now rests on a measured, working transfer |
| 2026-08-03 | Syscall coverage complete-or-blocked: **12/16 measured, 4 blocked, 0 untouched** — 0x0E set_flags −41 (flags 0 and 1), 0x0F create_eoa −22 (NULL sig/proof), plus 0x08/0x09 −43 | All four blocked on undocumented *argument shapes*, not authorization | None needed permanent creates to probe: upgrade + ephemeral accounts sufficed during the desync |
| 2026-08-03 | 6,303 vs ≈2,611 ephemeral-create "discrepancy" is two different quantities: whole-transaction single create (floor incl., no resize) vs marginal batch cost (incl. writable+resize, floor amortized) | Headline figures published without their conditions invite false contradiction | Both published with conditions in 12's README; like-for-like residual ~108 CU (codegen, UNVERIFIED) |
| 2026-08-03 | "0.18% of the compute budget" was ambiguous: it's against the CLI's **300M default request**, not the 4.29B u32 max (0.0125%) — denominators differ 14× | Percentages of "the budget" without naming the denominator | 09 README now names the basis; always state which ceiling a % is against |
| 2026-08-03 | bench/estimate.py: additive model predicts **89/89 recorded rows exactly** after per-family instruction-term calibration | The model's constants are real; only the instruction term is program-specific | Calibrate one size per program, then predict everything; excluded classes listed in the tool output (31 calibration rows behind the 89 — the tool now prints the per-family ratio) |
| 2026-08-04 | The recorded `upgrade_cu` figures were **4-txn subtotals — the pipeline runs 5 transactions** and the chunk-write signature is never printed | The already-logged create-pipeline gotcha ("chunk-write sig not shown") was never applied to our own upgrade-measuring grep | A logged gotcha you don't grep your own tooling for will bite twice; all six breakdowns reconstructed from chain history, results.json now carries per-stage figures |
| 2026-08-04 | **Upgrade pipeline is fully deterministic** — 3 same-binary upgrades: five identical stages to the CU; stages reproduce across 110,000 slots. The -Os/-Oz "1,280 CU same-size difference" localizes to the Step-2 upgrade txn: **grow 1,280→1,344 (SU 1) vs overwrite 1,344→1,344 (SU 0)** | The upgrade step's cost is a deterministic function of the old→new size transition; even finalize (which processes full content) is content-independent | Yesterday's "irreducible per-instance term" RETRACTED same-day-plus-one; stage laws: create 11,723+1/B exact, chunk 34,514+4.75/B exact, finalize ≈165–168/B, cleanup 35,782 flat |
| 2026-08-04 | The temp-buffer account's seed is deterministic, so **one account's history archives every past pipeline run** — including the never-printed chunk-write sigs | `program upgrade` derives the buffer from the program seed | `thru account transactions <buffer>` is the pipeline's flight recorder; also: the CLI caps account history at 50 txns — Explorer `list_account_transactions` paginates past it |
| 2026-08-04 | **Compress proof term VERIFIED, decompress refit CLOSED — from history alone, no new txns**: compress instr data = proof+3, CU−instr−5,850 = size at 13/13; decompress = **5,911 + revived + proof** exact ×3 (instr = revived+proof+43) | The proof sizes the CLI "never prints" were sitting in every transaction's own instruction data | Before calling a quantity unobservable, check whether it is literally the instruction payload; the 6,111/6,079/6,111 "non-constant remainders" were proof variation — absorbed-term error class, occurrence #4 |
| 2026-08-04 | **Envelope-limited, not compute-limited** (now the repo headline): heaviest measured CU 2,480,667 = 0.058% of ceiling, SU 16 = 0.024%, MU 1,001 = 1.5%, while txn size hit 32,760/32,768 = 99.98%; no recorded row's budget ratio ever beat its size ratio | Each declared account costs 64× more of the size budget than of the MU budget | Caveat kept honest: a 16 MiB resize (SU 4,096 = 6.25%) is the one unmeasured shape that could flip the ordering — testable without creates via an existing permanent account |
